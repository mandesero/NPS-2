...
from .cluster_tools import send_mininet_ping_to_cluster_node, send_mininet_cmd_to_cluster_node
from config import CLI_PROMPT_STRING, DST_SCRIPT_FOLDER, INFECTED_HOSTS_FILENAME
...
class CLI_director(cmd.Cmd):
    ...
    def do_clusterinfo(self, args):
        print(
            "Number of nodes in graph is "
            + str(len(self.hosts.keys()) + self.cluster_info["switch_number"])
            + "."
        )
