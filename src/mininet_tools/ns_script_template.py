...
def gen_mn_ns_script_by_template_with_custom_host_ip(
    ...
    # file.write('''                     sw.name, '\\n') \n''')
    # file.write('''                _intf = Intf(intfName, node=sw) \n''')
    file.write(f'''        ng_ip = {str(spec_group_ips)}\n''')
    file.write('''        count = 0\n''')
    ...
    file.write('''                other = s_to\n''')
    file.write('''            elif s_to in (sw.name for sw in mn.switches):\n''')
    file.write('''                S = mn[s_to]\n''')
    file.write('''                name = s_to\n''')
    file.write('''                other = s_from\n''')
    file.write('''            else:\n''')
    file.write('''                continue\n''')
    file.write('''                \n''')
    file.write('''            S.cmd(f'ovs-vsctl add-port {name} {name}-gre{count} -- set interface {name}-gre{count}     type=gre options:remote_ip={ng_ip[int(other[1:])]}')\n''')
    file.write('''            _ = Intf(f"{name}-gre{count}", node=S)\n''')
    file.write(''' \n''')
    file.write('''        info('*** Note: you may need to reconfigure the interfaces for ' \n''')
    file.write('''             'the Mininet hosts:\\n', mn.hosts, '\\n') \n''')
    file.write(''' \n''')
